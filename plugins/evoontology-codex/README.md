...
`visualize_ontology`、`evolution_status` 与进化会话工具完成，无需在用户项目中运行
...
- 说「构建本体层」或 `/evo-build` —— 按 `evo-build` skill 构建 `ontology_v0`；
- 说「进化本体层」或 `/evo-evolve` —— 按 `evo-evolve` skill 执行
...
- 说「可视化本体层」或 `/evo-visualize` —— 按 `evo-visualize` skill 生成离线交互图。
...
- `rolling_trajectory`：用于真实业务或冷启动；seed workload 初始化本体层，后续按 checkpoint
